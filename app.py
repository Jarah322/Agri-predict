...
def predict_yield(region, crop, year, temperature, rainfall, pesticide):
    ...
    try:
        ...
        input_data = np.array([[year, rainfall, pesticide, temperature, region, crop]], dtype=object)
...
def predict():
    try:
        ...
        # Input validation
        if not all([region, crop, year, temperature, rainfall, pesticide]):
            return jsonify({'error': 'All fields are required'}), 400
        
        # Make prediction using the trained model
        prediction, confidence = predict_yield(region, crop, year, temperature, rainfall, pesticide)
